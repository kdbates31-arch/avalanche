...
function ContactForm({ form, setForm, cardPhotos, setCardPhotos, officeOptions = [], onSubmit, saving, nextFollowUp, today, buttonText }) {
  ...
  function handleRelationshipStatusChange(newStatus) {
    const actionDate = today();
    setForm({
      ...form,
      relationship_status: newStatus,
      last_contact_date: actionDate,
      next_follow_up_date: nextFollowUp(newStatus, actionDate) || ""
    });
  }

  function handleLastContactChange(newDate) {
    setForm({
      ...form,
      last_contact_date: newDate,
      next_follow_up_date: nextFollowUp(form.relationship_status, newDate) || ""
    });
  }
  ...
  )}<Select label="Relationship Status" value={form.relationship_status} options={STATUSES} onChange={handleRelationshipStatusChange} /><Input label="Company Name" value={form.agency_name} onChange={(v) => set("agency_name", v)} required /><Input label="First Name" value={form.agent_first_name} onChange={(v) => set("agent_first_name", v)} /><Input label="Last Name" value={form.agent_last_name} onChange={(v) => set("agent_last_name", v)} /><Input label="Mobile Phone" value={form.agent_phone} onChange={(v) => set("agent_phone", v)} /><Input label="Office Phone" value={form.main_phone} onChange={(v) => set("main_phone", v)} /><Input label="Email" value={form.agent_email} onChange={(v) => set("agent_email", v)} /><Input label="Office Address" value={form.address} onChange={(v) => set("address", v)} /><Input label="City" value={form.city} onChange={(v) => set("city", v)} /><Input label="State" value={form.state} onChange={(v) => set("state", v)} /><Select label="Company Type" value={form.company_type} options={COMPANY_TYPES} onChange={(v) => set("company_type", v)} /><DatalistInput label="Contact Role" value={form.contact_role} onChange={(v) => set("contact_role", v)} suggestions={CONTACT_ROLE_SUGGESTIONS} /><Select label="Contact Origin" value={form.referral_source} options={CONTACT_ORIGINS} onChange={(v) => set("referral_source", v)} /><Select label="Trust Level" value={form.relationship_strength} options={RELATIONSHIP_STRENGTH} onChange={(v) => set("relationship_strength", v)} /><BusinessCardUpload cardPhotos={cardPhotos} setCardPhotos={setCardPhotos} /><Input label="Introduced By" value={form.introduced_by} onChange={(v) => set("introduced_by", v)} /><Input type="date" label="Last Contact" value={form.last_contact_date} onChange={handleLastContactChange} /><Input type="date" label="Scheduled Follow Up" value={form.next_follow_up_date} onChange={(v) => set("next_follow_up_date", v)} /><Input label="Favorite Food / Drink" value={form.favorite_food} onChange={(v) => set("favorite_food", v)} /><Input label="Birthday" value={form.birthday} onChange={(v) => set("birthday", v)} /><Input label="Tags" value={form.tags} onChange={(v) => set("tags", v)} /><Textarea label="Memory Notes" value={form.notes} onChange={(v) => set("notes", v)} /><button style={actionStyle("save", true)} disabled={saving}>{saving ? "Saving..." : buttonText}</button></form>;
}
