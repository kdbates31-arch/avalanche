...
export default function App() {
  ...
  async function saveReferralLead(e) {
    ...
    try {
      ...
      const lead = {
        id: makeId(),
        sent_by_id: referralForm.sent_by_id || "",
      ...
      if (!data) {
        setMessage("Referral Lead save failed: Supabase did not return the new referral record.");
        return;
      }

      setReferralForm({ ...EMPTY_REFERRAL, created_at: today(), updated_at: today(), referral_status: "New Referral" });
      setAddMode("relationship");
      setReferrals((current) => [data, ...current.filter((item) => item.id !== data.id)]);
      await loadReferrals();
      goToTab("referrals");
