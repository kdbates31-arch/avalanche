...
${body}`,
  ...
  const recentActivity = useMemo(() => {
    ...
    Object.entries(logs).forEach(([agencyId, items]) => {
      ...
      (items || []).forEach((item) => {
        allLogs.push({
          ...item,
          contact_name: agent ? agent.name : "Unknown Contact",
          company_name: agent ? agent.agency_name : "Unknown Company"
        });
      });
    });
    return allLogs
      .sort((a, b) => String(b.engagement_date || "").localeCompare(String(a.engagement_date || "")))
      .slice(0, 8);
  }, [logs, enrichedAgents]);
  ...
  const referralSummary = useMemo(() => {
    return REFERRAL_SOURCES.map((source) => ({
      source,
      count: enrichedAgents.filter((a) => a.referral_source === source).length
    })).filter((item) => item.count > 0);
  }, [enrichedAgents]);

  const strengthSummary = useMemo(() => {
    return RELATIONSHIP_STRENGTH.map((strength) => ({
      strength,
      count: enrichedAgents.filter((a) => a.relationship_strength === strength).length
    })).filter((item) => item.count > 0);
  }, [enrichedAgents]);
    ...
    <div style={styles.app}>
      <main style={styles.screen}>
        ...
        {!loading && tab === "mission" && (
          <div style={styles.stack}>
            ...
            {referralSummary.length > 0 && (
              <Section title="Referral Source Snapshot" subtitle="Where your relationship pipeline is coming from.">
                <div style={styles.summaryGrid}>
                  {referralSummary.map((item) => (
                    <button key={item.source} style={styles.summaryCard} onClick={() => { setReferralFilter(item.source); setStatusFilter("All"); setCompanyFilter("All"); setStrengthFilter("All"); setRoleFilter("All"); setSearch(""); setTab("contacts"); }}>
            ...
            {strengthSummary.length > 0 && (
              <Section title="Relationship Strength Snapshot" subtitle="How strong your current network feels.">
                <div style={styles.summaryGrid}>
                  {strengthSummary.map((item) => (
                    <button key={item.strength} style={styles.summaryCard} onClick={() => { setStrengthFilter(item.strength); setStatusFilter("All"); setCompanyFilter("All"); setReferralFilter("All"); setRoleFilter("All"); setSearch(""); setTab("contacts"); }}>
              ...
              {recentActivity.length === 0 ? (
                ...
                recentActivity.map((item) => (
                  <div key={item.id} style={styles.timelineItem}>
                    <b>{item.engagement_type}</b>
                    <p>{item.contact_name} · {item.company_name}</p>
                    <small>{item.engagement_date}</small>
        ...
        {!loading && tab === "partners" && (
          <div style={styles.stack}>
            <Section title="Active Referral Partners" subtitle="Referral relationships that are already producing or actively trusted.">
              {activeAgents.filter((agent) => agent.relationship_status === "Active Referral Partner").length === 0 ? (
                <p>No active referral partners yet.</p>
              ) : (
                activeAgents
                  .filter((agent) => agent.relationship_status === "Active Referral Partner")
                  .map((agent) => <ContactRow key={agent.id} agent={agent} open={() => setSelectedAgent(agent)} openCall={() => setCallAgent(agent)} emailAndLog={() => emailAndLog(agent)} />)
              )}
            </Section>

            <Section title="VIP Referral Partners" subtitle="Strategic relationships worth protecting intentionally.">
              {activeAgents.filter((agent) => agent.relationship_status === "VIP Referral Partner").length === 0 ? (
                <p>No VIP referral partners yet.</p>
              ) : (
                activeAgents
                  .filter((agent) => agent.relationship_status === "VIP Referral Partner")
                  .map((agent) => <ContactRow key={agent.id} agent={agent} open={() => setSelectedAgent(agent)} openCall={() => setCallAgent(agent)} emailAndLog={() => emailAndLog(agent)} soft />)
              )}
            </Section>
          </div>
        )}
