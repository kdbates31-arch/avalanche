...
export default function App() {
  ...
  async function saveAgent(e) {
    ...
    const payload = { ...agentForm, last_contact_date: last, next_follow_up_date: next };
  ...
  const dueNow = activeAgents.filter((a) => a.relationship_status !== "VIP Referral Partner" && isContactDueByStatus(a));
  const follow15 = activeAgents.filter((a) => ["New Prospect", "Needs Follow Up"].includes(a.relationship_status) && isDateTriggeredActionDue(a.last_contact_date, 15));
  const warm30 = activeAgents.filter((a) => a.relationship_status === "Warm Relationship" && isDateTriggeredActionDue(a.last_contact_date, 30));
  const maintenance60 = activeAgents.filter((a) => a.relationship_status === "Maintenance Relationship" && isDateTriggeredActionDue(a.last_contact_date, 60));
  const vipSuggestions = activeAgents.filter((a) => a.relationship_status === "VIP Referral Partner" && isDateTriggeredActionDue(a.last_contact_date, 45));
  ...
  const referralSummary = useMemo(() => {
    return REFERRAL_SOURCES.map((source) => ({ source, count: enrichedAgents.filter((a) => a.referral_source === source).length })).filter((item) => item.count > 0);
  }, [enrichedAgents]);
  ...
  const directory = useMemo(() => {
    ...
    if (statusFilter === "15 Day Due") list = list.filter((a) => ["New Prospect", "Needs Follow Up"].includes(a.relationship_status) && isDateTriggeredActionDue(a.last_contact_date, 15));
    ...
    if (referralFilter !== "All") list = list.filter((a) => a.referral_source === referralFilter);
  ...
  function filterReferral(source) {
    resetFilters();
  ...
  return (
    <div style={styles.app}>
      <main style={styles.screen}>
        <header style={styles.header}>
          <div>
            ...
            <h1 style={styles.h1}>{tab === "mission" ? "Today's Mission" : tab === "contacts" ? "Contacts" : tab === "add" ? "Add Contact" : tab === "partners" ? "Partners" : "Playbook"}</h1>
          </div>
        ...
        {!loading && tab === "mission" && (
          <div style={styles.stack}>
            <div style={styles.tiles}>
              <Tile label="Needs Attention" value={dueNow.length} onClick={() => openDueBucket("All")} danger />
              <Tile label="15+ Day Touches" value={follow15.length} onClick={() => openDueBucket("15 Day Due")} />
              <Tile label="30+ Day Touches" value={warm30.length} onClick={() => openDueBucket("30 Day Due")} />
              <Tile label="60+ Day Touches" value={maintenance60.length} onClick={() => openDueBucket("60 Day Due")} />
              <Tile label="VIP Relationship Touches" value={vipSuggestions.length} onClick={() => openDueBucket("VIP Suggestions")} />
            </div>
            ...
            {referralSummary.length > 0 && <SummarySection title="Referral Source Snapshot" subtitle="Where your relationship pipeline is coming from." items={referralSummary} labelKey="source" onClick={filterReferral} />}
            {strengthSummary.length > 0 && <SummarySection title="Relationship Strength Snapshot" subtitle="How strong your current network feels." items={strengthSummary} labelKey="strength" onClick={filterStrength} />}
            ...
            <Section title="VIP Relationship Touches" subtitle="Soft reminders, not sales pressure.">
              {vipSuggestions.length === 0 ? <p>No VIP relationship touches suggested right now.</p> : vipSuggestions.slice(0, 10).map((agent) => <ContactRow key={agent.id} agent={agent} open={() => setSelectedAgent(agent)} openCall={() => setCallAgent(agent)} emailAndLog={() => emailAndLog(agent)} soft />)}
            </Section>
        ...
        {!loading && tab === "contacts" && (
          <div style={styles.stack}>
            <div style={styles.card}>
              <button style={styles.primary} onClick={resetFilters}>Show All Contacts</button>
              ...
              <select style={styles.input} value={referralFilter} onChange={(e) => setReferralFilter(e.target.value)}><option>All</option>{REFERRAL_SOURCES.map((x) => <option key={x}>{x}</option>)}</select>
              <select style={styles.input} value={strengthFilter} onChange={(e) => setStrengthFilter(e.target.value)}><option>All</option>{RELATIONSHIP_STRENGTH.map((x) => <option key={x}>{x}</option>)}</select>
              <select style={styles.input} value={roleFilter} onChange={(e) => setRoleFilter(e.target.value)}><option>All</option>{roleOptions.map((x) => <option key={x}>{x}</option>)}</select>
            </div>
            <Section title={`Contact Directory (${directory.length})`} subtitle="Search or filter by status, company type, referral source, role, or strength.">
        ...
        {!loading && tab === "add" && <ContactForm form={agentForm} setForm={setAgentForm} onSubmit={saveAgent} saving={saving} nextFollowUp={nextFollowUp} today={today} buttonText="Save Contact" />}
      ...
      {selectedAgent && <ContactProfile agent={selectedAgent} logs={logs[selectedAgent.id] || []} close={() => setSelectedAgent(null)} startEdit={startEdit} deleteAgent={deleteAgent} moveToDoNotPursue={moveToDoNotPursue} openCall={() => setCallAgent(selectedAgent)} emailAndLog={() => emailAndLog(selectedAgent)} startLog={startLog} />}
      ...
      {editingAgent && <EditModal form={editingAgent} setForm={setEditingAgent} updateAgent={updateAgent} saving={saving} close={() => setEditingAgent(null)} nextFollowUp={nextFollowUp} today={today} />}
    </div>
  );
}
...
function SummarySection({ title, subtitle, items, labelKey, onClick }) {
  return <Section title={title} subtitle={subtitle}><div style={styles.summaryGrid}>{items.map((item) => <button key={item[labelKey]} style={styles.summaryCard} onClick={() => onClick(item[labelKey])}><span>{item[labelKey]}</span><b>{item.count}</b></button>)}</div></Section>;
}
...
function ContactRow({ agent, open, openCall, emailAndLog, soft }) {
  const label = agent.days === null ? "No trigger" : agent.days < 0 ? `${Math.abs(agent.days)} days past` : agent.days === 0 ? "Ready for touch" : `${agent.days} days left`;
  return (
    ...
    <div style={styles.overlay}>
      <div style={styles.sheet}>
        ...
        <div style={styles.actionRow}>
          ...
          <button style={styles.smallButton} onClick={() => startEdit(agent)}>Edit</button>
          <button style={styles.warningButton} onClick={() => moveToDoNotPursue(agent)}>Do Not Pursue</button>
          <button style={styles.deleteButton} onClick={() => deleteAgent(agent)}>Delete</button>
        </div>
        <div style={styles.actionRow}>
...
function CallChoiceModal({ agent, close, startLog }) {
  return <div style={styles.overlay}><div style={styles.sheet}><button style={styles.smallButton} onClick={close}>Cancel</button><h2>Choose Number</h2><p>{agent.name} · {agent.agency_name}</p>{agent.agent_phone ? <a style={styles.callButton} href={`tel:${agent.agent_phone}`}>Call Mobile: {agent.agent_phone}</a> : <div style={styles.disabledBox}>No mobile phone saved</div>}{agent.main_phone ? <a style={styles.callButton} href={`tel:${agent.main_phone}`}>Call Office: {agent.main_phone}</a> : <div style={styles.disabledBox}>No office phone saved</div>}<button style={styles.primary} onClick={() => { close(); startLog(agent, "Phone Call"); }}>Log Phone Call Notes</button></div></div>;
}

function ActivityLogModal({ agent, type, form, setForm, saveActivity, saving, close }) {
  function set(key, value) { setForm({ ...form, [key]: value }); }
  return <div style={styles.overlay}><div style={styles.sheet}><button style={styles.smallButton} onClick={close}>Cancel</button><h2>{type}</h2><p>{agent.name} · {agent.agency_name}</p><form style={styles.form} onSubmit={saveActivity}>{type === "Office Stop In" ? <Select label="Action Taken" value={form.action_taken} options={["Coffee", "Brought Food", "Tacos", "Lunch Invite", "Dropped Marketing Material", "Met New Staff", "Other"]} onChange={(v) => set("action_taken", v)} /> : <Input label="Action Taken" value={form.action_taken} onChange={(v) => set("action_taken", v)} />}<Textarea label="Activity Notes" value={form.notes} onChange={(v) => set("notes", v)} /><Textarea label="Outcome" value={form.outcome} onChange={(v) => set("outcome", v)} /><Input label="Next Action" value={form.next_action} onChange={(v) => set("next_action", v)} /><button style={styles.primary} disabled={saving}>{saving ? "Saving..." : "Save Activity"}</button></form></div></div>;
}

function EditModal({ form, setForm, updateAgent, saving, close, nextFollowUp, today }) {
  return <div style={styles.overlay}><div style={styles.sheet}><button style={styles.smallButton} onClick={close}>Cancel</button><h2>Edit Contact</h2><ContactForm form={form} setForm={setForm} onSubmit={updateAgent} saving={saving} nextFollowUp={nextFollowUp} today={today} buttonText="Update Contact" /></div></div>;
}

function ContactForm({ form, setForm, onSubmit, saving, nextFollowUp, today, buttonText }) {
  function set(key, value) { setForm({ ...form, [key]: value }); }
  return <form style={styles.form} onSubmit={onSubmit}><Select label="Company Type" value={form.company_type} options={COMPANY_TYPES} onChange={(v) => set("company_type", v)} /><Input label="Contact Role" value={form.contact_role} onChange={(v) => set("contact_role", v)} /><Select label="Referral Source" value={form.referral_source} options={REFERRAL_SOURCES} onChange={(v) => set("referral_source", v)} /><Select label="Relationship Strength" value={form.relationship_strength} options={RELATIONSHIP_STRENGTH} onChange={(v) => set("relationship_strength", v)} /><Input label="Introduced By" value={form.introduced_by} onChange={(v) => set("introduced_by", v)} /><Input label="Company Name" value={form.agency_name} onChange={(v) => set("agency_name", v)} required /><Input label="First Name" value={form.agent_first_name} onChange={(v) => set("agent_first_name", v)} /><Input label="Last Name" value={form.agent_last_name} onChange={(v) => set("agent_last_name", v)} /><Input label="Mobile Phone" value={form.agent_phone} onChange={(v) => set("agent_phone", v)} /><Input label="Office Phone" value={form.main_phone} onChange={(v) => set("main_phone", v)} /><Input label="Email" value={form.agent_email} onChange={(v) => set("agent_email", v)} /><Input label="Address" value={form.address} onChange={(v) => set("address", v)} /><Input label="City" value={form.city} onChange={(v) => set("city", v)} /><Input label="State" value={form.state} onChange={(v) => set("state", v)} /><Select label="Relationship Status" value={form.relationship_status} options={STATUSES} onChange={(v) => setForm({ ...form, relationship_status: v, next_follow_up_date: nextFollowUp(v, form.last_contact_date || today()) || "" })} /><Input type="date" label="Last Contact" value={form.last_contact_date} onChange={(v) => setForm({ ...form, last_contact_date: v, next_follow_up_date: nextFollowUp(form.relationship_status, v) || "" })} /><Input type="date" label="Next Follow Up" value={form.next_follow_up_date} onChange={(v) => set("next_follow_up_date", v)} /><Input label="Favorite Food / Drink" value={form.favorite_food} onChange={(v) => set("favorite_food", v)} /><Input label="Birthday" value={form.birthday} onChange={(v) => set("birthday", v)} /><Input label="Tags" value={form.tags} onChange={(v) => set("tags", v)} /><Textarea label="Memory Notes" value={form.notes} onChange={(v) => set("notes", v)} /><button style={styles.primary} disabled={saving}>{saving ? "Saving..." : buttonText}</button></form>;
}

function Playbook() {
  return <Section title="Playbook" subtitle="Current cadence logic"><p><b>New Prospect:</b> 15+ day relationship touch.</p><p><b>Needs Follow Up:</b> 15+ day relationship touch.</p><p><b>Warm Relationship:</b> 30+ day relationship touch.</p><p><b>Maintenance Relationship:</b> 60+ day relationship touch.</p><p><b>Active Referral Partner:</b> no automated sales trigger.</p><p><b>VIP Referral Partner:</b> 45+ day soft relationship touch.</p></Section>;
}
...
const styles = {
  ...
  sub: { color: "#94a3b8" },
  row: { background: "#1e293b", padding: 12, borderRadius: 16, display: "flex", flexDirection: "column", gap: 4 },
  ...
  navButton: { padding: 10, borderRadius: 14, background: "transparent", color: "#94a3b8", border: 0, fontWeight: 800, fontSize: 12 },
  navOn: { padding: 10, borderRadius: 14, background: "white", color: "#020617", border: 0, fontWeight: 800, fontSize: 12 },
