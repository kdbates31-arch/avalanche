...
export default function App() {
  ...
  async function updateReferralLead(e) {
    ...
    if (payload.referral_status === "Lost") {
      payload.lost_reason = payload.lost_reason || "Insurance Denied";
      payload.lost_drip_step = payload.lost_drip_step || "30";
      payload.lost_drip_next_date = payload.lost_drip_next_date || addDays(today(), 30);
    } else {
      payload.lost_drip_next_date = null;
      payload.lost_drip_last_sent_date = null;
      payload.lost_drip_step = null;
      payload.lost_reason = payload.lost_reason || "";
    }
  ...
  async function updateReferralStatus(referral, nextStatus) {
    ...
    if (nextStatus === "Lost") {
      const existingReason = referral.lost_reason && LOST_REASONS.includes(referral.lost_reason) ? referral.lost_reason : "";
      const promptedReason = existingReason || window.prompt(`Lost reason?\n\nType one exactly:\n${LOST_REASONS.join("\n")}`, "Insurance Denied");
      const finalReason = LOST_REASONS.includes(String(promptedReason || "").trim()) ? String(promptedReason).trim() : "Insurance Denied";
      updates.lost_reason = finalReason;
    ...
    if (["Contract Signed", "Working", "Estimate Scheduled", "Archived"].includes(nextStatus)) {
  ...
  return (
    <div style={{ ...styles.row, borderLeft: `6px solid ${TILE_ACCENTS.sync}` }}>
      <div onClick={open} style={{ cursor: "pointer" }}>
        ...
        <div style={styles.actionRowSmall}><ReferralStatusBadge status={status} />{status === "Lost" && <small>{referral.lost_reason || "Insurance Denied"}</small>}</div>
      </div>
      ...
      <Section title="Referral Status" subtitle="Track what happened to this referral lead.">
        <Select label="Referral Status" value={form.referral_status || "New Referral"} options={REFERRAL_STATUSES} onChange={(v) => {
          ...
          if (v === "Lost") {
            ...
            next.lost_drip_step = next.lost_drip_step || "30";
            next.lost_drip_next_date = next.lost_drip_next_date || addDaysStatic(todayStatic(), 30);
          }
          setForm(next);
        }} />
        {(form.referral_status || "New Referral") === "Lost" && <Select label="Lost Reason" value={form.lost_reason || "Insurance Denied"} options={LOST_REASONS} onChange={(v) => set("lost_reason", v)} />}
      </Section>
