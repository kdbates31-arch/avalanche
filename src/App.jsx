...
export default function App() {
  ...
  const [contactReferralStatusFilter, setContactReferralStatusFilter] = useState("Working");
  ...
  async function deleteReferralLead(referral) {
    if (!referral?.id) return;
    const confirmed = window.confirm("Permanently delete this Referral Lead? This cannot be undone. Use this only for duplicates, test leads, spam, or wrong entries.");
    if (!confirmed) return;
    ...
    if (error) {
      setMessage("Referral Lead delete failed: " + error.message);
      return;
    }
    ...
    await loadReferrals();
    setMessage("Referral Lead deleted permanently.");
  }
  ...
  const contactReferralTiles = [
    { label: "Working Leads", status: "Working", value: referrals.filter((item) => (item.referral_status || "New Referral") === "Working").length, accent: TILE_ACCENTS.fifteen },
    { label: "Estimate Process", status: "Estimate Process", value: referrals.filter((item) => (item.referral_status || "New Referral") === "Estimate Process").length, accent: TILE_ACCENTS.thirty },
    { label: "Signed Contacts", status: "Contract Signed", value: signedReferralLeads.length, accent: "#22c55e" },
    { label: "Lost Contacts", status: "Lost", value: lostReferralLeads.length, accent: "#ef4444" }
  ];

  const contactReferralLeads = referrals
    .filter((item) => (item.referral_status || "New Referral") === contactReferralStatusFilter)
    .filter((item) => referralMatchesSearch(item, search.trim().toLowerCase()))
    .sort((a, b) => String(b.updated_at || b.created_at || "").localeCompare(String(a.updated_at || a.created_at || "")));

  const recentUpdatedRecords = useMemo(() => {
    const relationshipRecords = enrichedAgents.map((agent) => ({
      type: "relationship",
      id: `relationship_${agent.id}`,
      date: agent.updated_at || agent.last_contact_date || agent.created_at || "",
      record: agent
    }));
    ...
    const referralRecords = referrals.map((referral) => ({
      ...
      date: referral.updated_at || referral.created_at || "",
      record: referral
    }));

    return [...relationshipRecords, ...referralRecords]
      .sort((a, b) => String(b.date || "").localeCompare(String(a.date || "")))
      .slice(0, 10);
  }, [enrichedAgents, referrals]);
  ...
  return (
    <div style={styles.app}>
        ...
        {!loading && tab === "contacts" && (
          <div style={styles.stack}>
            <div style={styles.card}>
              <h2>Contacts</h2>
              <p style={styles.sub}>Search first, then use the tiles below so the screen stays clean instead of dumping every referral lead at the top.</p>
              <input style={styles.input} value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search leads, homeowners, relationship contacts, companies, roles, phones, emails..." />

              <div style={styles.tiles}>
                {contactReferralTiles.map((tile) => (
                  <Tile
                    key={tile.status}
                    label={tile.label}
                    value={tile.value}
                    accent={tile.accent}
                    onClick={() => { setContactReferralStatusFilter(tile.status); resetView(); }}
                  />
                ))}
              </div>
            </div>

            <Section title={`${contactReferralTiles.find((tile) => tile.status === contactReferralStatusFilter)?.label || "Referral Leads"} (${contactReferralLeads.length})`} subtitle="These are grouped by status so the Contacts screen does not become a mile-long scroll monster.">
              {contactReferralLeads.length === 0 ? <p>No referral leads match this tile and search.</p> : contactReferralLeads.map((ref) => <ReferralRow key={ref.id} referral={ref} open={() => { resetView(); setSelectedReferral(ref); }} updateReferralStatus={updateReferralStatus} sendLostDripEmail={sendLostDripEmail} />)}
            </Section>

            <Section title="Recent / Recently Updated" subtitle="Fast access to the records you just touched, because last-minute adjustments are where the gremlins live.">
              {recentUpdatedRecords.length === 0 ? <p>No recent records yet.</p> : recentUpdatedRecords.map((item) => (
                <RecentUpdatedRow
                  key={item.id}
                  item={item}
                  openReferral={(ref) => { resetView(); setSelectedReferral(ref); }}
                  openContact={(agent) => { resetView(); setSelectedAgent(agent); }}
                />
              ))}
            </Section>
      ...
      {selectedReferral && <ReferralProfile referral={selectedReferral} close={() => setSelectedReferral(null)} edit={() => { setEditingReferral(selectedReferral); setSelectedReferral(null); }} updateReferralStatus={updateReferralStatus} archiveReferral={archiveReferral} deleteReferralLead={deleteReferralLead} sendLostDripEmail={sendLostDripEmail} />}
...
function RecentUpdatedRow({ item, openReferral, openContact }) {
  if (item.type === "referral") {
    const referral = item.record;
    return (
      <div style={{ ...styles.row, borderLeft: "6px solid #a78bfa" }} onClick={() => openReferral(referral)}>
        <b>{referralCustomerName(referral)}</b>
        <p>{referralAddress(referral) || "No property address saved"}</p>
        <small>Referral Lead · {referral.referral_status || "New Referral"} · Updated {String(item.date || "Not dated").slice(0, 10)}</small>
      </div>
    );
  }

  const agent = item.record;
  const statusColor = getStatusColor(agent.relationship_status);
  return (
    <div style={{ ...styles.row, borderLeft: `6px solid ${statusColor.accent}` }} onClick={() => openContact(agent)}>
      <b>{agent.name}</b>
      <p>{agent.agency_name || "No company saved"}</p>
      <small>Relationship Contact · {agent.relationship_status || "No Status"} · Updated {String(item.date || "Not dated").slice(0, 10)}</small>
    </div>
  );
}
...
function ReferralProfile({ referral, close, edit, updateReferralStatus, archiveReferral, deleteReferralLead, sendLostDripEmail }) {
  const status = referral.referral_status || "New Referral";
  return (
    <div style={styles.overlay}>
      <div style={styles.sheet}>
        ...
        <div style={styles.actionRow}>
          ...
          <button style={actionStyle("caution")} onClick={() => archiveReferral(referral)}>Archive</button>
          <button style={actionStyle("delete")} onClick={() => deleteReferralLead(referral)}>Delete</button>
        </div>
      </div>
    </div>
  );
