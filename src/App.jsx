...
async function uploadBusinessCardImage(value, folder, side) {
  ...
  if (error) throw new Error(`Business card ${side} upload failed: ${error.message}. Confirm the business-cards bucket exists, is public, and has insert/select policies for anon.`);
...
export default function App() {
    ...
    try {
      ...
      if (newId && (agentCardPhotos.front || agentCardPhotos.back)) {
        try {
          ...
          if (photoUpdateError) {
            setMessage("Contact saved, but business card photo links failed: " + photoUpdateError.message);
          } else {
            setMessage("Relationship Contact saved with business card photos.");
          }
        } catch (photoError) {
          setMessage("Contact saved, but business card photos failed: " + (photoError?.message || String(photoError)));
        }
      } else {
  ...
  async function updateAgent(e) {
    ...
    try {
      ...
      let finalFront = existingFront;
      let finalBack = existingBack;

      if (editingCardPhotos.front || editingCardPhotos.back) {
        const uploadedPhotos = await uploadBusinessCardPhotos(editingCardPhotos, String(editingAgent.id || "contact"));
        finalFront = uploadedPhotos.front.url || "";
        finalBack = uploadedPhotos.back.url || "";
      }

      payload.business_card_front = finalFront;
      payload.business_card_back = finalBack;
