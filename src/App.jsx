...
const SORT_OPTIONS = ["Most Overdue", "Highest Score", "Lowest Score", "Name A-Z", "Recently Contacted"];
...
export default function App() {
  ...
  function scoreAgent(agent, history = []) {
    let score = 40;
    ...
    score += Math.min(20, history.length * 4);
  ...
  async function loadDashboard() {
    ...
    if (engagementError) setMessage("Database error loading engagement history: " + engagementError.message);
  ...
  async function saveEngagement(e) {
    ...
    if (!engagementForm.notes.trim()) return setMessage("Notes are required for an engagement.");
    ...
    if (updateError) setMessage("Engagement saved, but agent update failed: " + updateError.message);
  ...
  async function updateStatus(agent, status) {
    const nextDate = calculateNextFollowUp(status, agent.last_contact_date || todayDateString());
  ...
  const enrichedAgents = useMemo(() => {
    return agents.map((agent) => {
      const history = engagements[agent.id] || [];
      return {
        ...agent,
        name: displayName(agent),
        history,
        score: scoreAgent(agent, history),
  ...
  const missionAgents = activeAgents.filter((agent) => agent.daysRemaining !== null && agent.daysRemaining <= 0).sort((a, b) => (a.daysRemaining ?? 9999) - (b.daysRemaining ?? 9999));
  ...
  const directoryAgents = useMemo(() => {
    ...
    if (statusFilter === "All Active") list = list.filter((agent) => (agent.relationship_status || "") !== "Do Not Pursue");
    else if (statusFilter === "Due Now") list = list.filter((agent) => (agent.relationship_status || "") !== "Do Not Pursue" && agent.daysRemaining !== null && agent.daysRemaining <= 0);
    else if (statusFilter === "15 Day Due") list = list.filter((agent) => ["Needs Follow Up", "New Prospect"].includes(agent.relationship_status) && agent.daysRemaining !== null && agent.daysRemaining <= 0);
    else if (statusFilter === "30 Day Due") list = list.filter((agent) => agent.relationship_status === "Warm Relationship" && agent.daysRemaining !== null && agent.daysRemaining <= 0);
    else if (statusFilter !== "All") list = list.filter((agent) => (agent.relationship_status || "") === statusFilter);

    if (term) {
      list = list.filter((agent) => [agent.name, agent.agency_name, agent.agent_phone, agent.main_phone, agent.agent_email, agent.city, agent.tags, agent.notes, agent.favorite_food].filter(Boolean).join(" ").toLowerCase().includes(term));
    }
  ...
  return (
    <div className="appShell">
      ...
      <main className="screen">
        ...
        {!loading && activeTab === "mission" && <MissionScreen missionAgents={missionAgents} followUpAgents={followUpAgents} warmAgents={warmAgents} partnerAgents={partnerAgents} openQuickAction={openQuickAction} openCallChoice={setCallChoiceAgent} openAgent={setSelectedAgent} openFilteredAgents={openFilteredAgents} emailLink={emailLink} />}
        {!loading && activeTab === "agents" && <AgentsScreen title={directoryTitle} agents={directoryAgents} search={search} setSearch={setSearch} statusFilter={statusFilter} setStatusFilter={setStatusFilter} sortBy={sortBy} setSortBy={setSortBy} setDirectoryTitle={setDirectoryTitle} openAgent={setSelectedAgent} openCallChoice={setCallChoiceAgent} emailLink={emailLink} />}
        {!loading && activeTab === "add" && <AddAgentScreen agentForm={agentForm} setAgentForm={setAgentForm} saveAgent={saveAgent} saving={saving} calculateNextFollowUp={calculateNextFollowUp} todayDateString={todayDateString} />}
        {!loading && activeTab === "partners" && <PartnersScreen agents={partnerAgents} openAgent={setSelectedAgent} openCallChoice={setCallChoiceAgent} emailLink={emailLink} />}
        {!loading && activeTab === "playbook" && <PlaybookScreen />}
      </main>

      <BottomNav activeTab={activeTab} setActiveTab={setActiveTab} />

      {selectedAgent && <AgentProfile agent={selectedAgent} engagements={engagements[selectedAgent.id] || []} close={() => setSelectedAgent(null)} openQuickAction={openQuickAction} openCallChoice={setCallChoiceAgent} updateStatus={updateStatus} archiveAgent={archiveAgent} deleteAgent={deleteAgent} emailLink={emailLink} />}
      {callChoiceAgent && <CallChoiceModal agent={callChoiceAgent} close={() => setCallChoiceAgent(null)} openQuickAction={openQuickAction} />}
      {quickActionAgent && <EngagementModal agent={quickActionAgent} actionType={quickActionType} form={engagementForm} setForm={setEngagementForm} saving={saving} saveEngagement={saveEngagement} close={() => setQuickActionAgent(null)} />}
    </div>
  );
}

function Header({ activeTab, onRefresh }) {
  const titles = { mission: "Today's Mission", agents: "Agent Directory", add: "Add Agent", partners: "Referral Partners", playbook: "Playbook" };
  return <div className="topHeader"><div><div className="eyebrow">Avalanche CRM</div><h1>{titles[activeTab]}</h1></div><button className="ghostButton" onClick={onRefresh}>Refresh</button></div>;
}

function MissionScreen({ missionAgents, followUpAgents, warmAgents, partnerAgents, openCallChoice, openAgent, openFilteredAgents, emailLink }) {
  ...
  return <div className="stack"><div className="metricGrid"><button className="metric metricButton dangerMetric" onClick={() => openFilteredAgents("Due Now", "All Follow-Ups Due Now")}><span>Due Today</span><strong>{missionAgents.length}</strong></button><button className="metric metricButton" onClick={() => openFilteredAgents("15 Day Due", "15-Day Follow-Ups Due")}><span>15-Day Due</span><strong>{fifteenDayDue.length}</strong></button><button className="metric metricButton" onClick={() => openFilteredAgents("30 Day Due", "30-Day Warm Touches Due")}><span>30-Day Due</span><strong>{thirtyDayDue.length}</strong></button><button className="metric metricButton" onClick={() => openFilteredAgents("Active Referral Partner", "Active Referral Partners")}><span>Partners</span><strong>{partnerAgents.length}</strong></button></div><Section title="Priority Calls" subtitle="Tap a tile above to open the full filtered list.">{missionAgents.length === 0 ? <EmptyState text="No overdue follow-ups." /> : missionAgents.slice(0, 12).map((agent) => <AgentRow key={agent.id} agent={agent} openAgent={openAgent} openCallChoice={openCallChoice} emailLink={emailLink} priority />)}</Section><Section title="Warm Relationship Watch" subtitle="30-day keep-warm cycle.">{warmAgents.length === 0 ? <EmptyState text="No warm relationships yet." /> : warmAgents.slice(0, 6).map((agent) => <AgentRow key={agent.id} agent={agent} openAgent={openAgent} openCallChoice={openCallChoice} emailLink={emailLink} />)}</Section></div>;
}
...
function AgentsScreen({ title, agents, search, setSearch, statusFilter, setStatusFilter, sortBy, setSortBy, setDirectoryTitle, openAgent, openCallChoice, emailLink }) {
  function showAllAgents() { setSearch(""); setStatusFilter("All"); setSortBy("Name A-Z"); setDirectoryTitle("All Saved Agents"); }
  return <div className="stack"><div className="filterCard"><button className="primaryButton smallPrimary" type="button" onClick={showAllAgents}>Show All Saved Agents</button><input className="searchInput" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search name, agency, city, notes, tags..." /><div className="filterRow"><select value={statusFilter} onChange={(e) => { setStatusFilter(e.target.value); setDirectoryTitle("Agent Directory"); }}><option>All</option><option>All Active</option><option>Due Now</option><option>15 Day Due</option><option>30 Day Due</option>{STATUS_OPTIONS.map((status) => <option key={status}>{status}</option>)}</select><select value={sortBy} onChange={(e) => setSortBy(e.target.value)}>{SORT_OPTIONS.map((item) => <option key={item}>{item}</option>)}</select></div></div><Section title={`${title || "Agent Directory"} (${agents.length})`} subtitle="Tap any row to view the full profile and history.">{agents.length === 0 ? <div className="emptyState"><p>No agents found for the current filter.</p><button className="secondaryFullButton" type="button" onClick={showAllAgents}>Reset and Show All Agents</button></div> : agents.map((agent) => <AgentRow key={agent.id} agent={agent} openAgent={openAgent} openCallChoice={openCallChoice} emailLink={emailLink} />)}</Section></div>;
}

function AddAgentScreen({ agentForm, setAgentForm, saveAgent, saving, calculateNextFollowUp, todayDateString }) {
  return <form className="formCard" onSubmit={saveAgent}><Input label="Agency Name" value={agentForm.agency_name} onChange={(value) => setAgentForm({ ...agentForm, agency_name: value })} required /><div className="twoCol"><Input label="First Name" value={agentForm.agent_first_name} onChange={(value) => setAgentForm({ ...agentForm, agent_first_name: value })} /><Input label="Last Name" value={agentForm.agent_last_name} onChange={(value) => setAgentForm({ ...agentForm, agent_last_name: value })} /></div><Input label="Mobile Phone" value={agentForm.agent_phone} onChange={(value) => setAgentForm({ ...agentForm, agent_phone: value })} /><Input label="Office Phone" value={agentForm.main_phone} onChange={(value) => setAgentForm({ ...agentForm, main_phone: value })} /><Input label="Email" value={agentForm.agent_email} onChange={(value) => setAgentForm({ ...agentForm, agent_email: value })} /><Input label="Address" value={agentForm.address} onChange={(value) => setAgentForm({ ...agentForm, address: value })} /><div className="twoCol"><Input label="City" value={agentForm.city} onChange={(value) => setAgentForm({ ...agentForm, city: value })} /><Input label="State" value={agentForm.state} onChange={(value) => setAgentForm({ ...agentForm, state: value })} /></div><Select label="Status" value={agentForm.relationship_status} options={STATUS_OPTIONS} onChange={(value) => setAgentForm({ ...agentForm, relationship_status: value, next_follow_up_date: calculateNextFollowUp(value, agentForm.last_contact_date || todayDateString()) || "" })} /><div className="twoCol"><Input label="Last Contact" type="date" value={agentForm.last_contact_date} onChange={(value) => setAgentForm({ ...agentForm, last_contact_date: value, next_follow_up_date: calculateNextFollowUp(agentForm.relationship_status, value) || "" })} /><Input label="Next Follow Up" type="date" value={agentForm.next_follow_up_date} onChange={(value) => setAgentForm({ ...agentForm, next_follow_up_date: value })} /></div><Input label="Favorite Food / Drink" value={agentForm.favorite_food} onChange={(value) => setAgentForm({ ...agentForm, favorite_food: value })} placeholder="Tacos, coffee, Diet Coke..." /><Input label="Birthday" value={agentForm.birthday} onChange={(value) => setAgentForm({ ...agentForm, birthday: value })} /><Input label="Tags" value={agentForm.tags} onChange={(value) => setAgentForm({ ...agentForm, tags: value })} placeholder="Responds to text, gatekeeper matters" /><Textarea label="Memory Notes" value={agentForm.notes} onChange={(value) => setAgentForm({ ...agentForm, notes: value })} placeholder="Personal details, preferences, relationship context." /><button className="primaryButton" disabled={saving}>{saving ? "Saving..." : "Save Agent"}</button></form>;
}

function PartnersScreen({ agents, openAgent, openCallChoice, emailLink }) {
  return <div className="stack"><Section title="Active Referral Partners" subtitle="These relationships should never be allowed to go cold.">{agents.length === 0 ? <EmptyState text="No referral partners yet." /> : agents.map((agent) => <AgentRow key={agent.id} agent={agent} openAgent={openAgent} openCallChoice={openCallChoice} emailLink={emailLink} />)}</Section></div>;
}

function PlaybookScreen() {
  return <div className="stack"><Play title="New Prospect" text="Make the first touch about usefulness, speed, and professionalism." /><Play title="Needs Follow Up" text="15-day cadence. Call or stop in. Learn one detail and log it." /><Play title="Warm Relationship" text="30-day cadence. Coffee, tacos, or quick check-in." /><Play title="Referral Partner" text="Protect the relationship. Thank them quickly and follow up professionally." /></div>;
}
...
function AgentRow({ agent, openAgent, openCallChoice, emailLink, priority }) {
  ...
  return <div className={`agentRow ${overdue || dueToday || priority ? "urgentRow" : ""}`}><div className="agentMain" onClick={() => openAgent(agent)}><div className="rowTop"><strong>{agent.name}</strong><span className="scorePill">{agent.score}</span></div><div className="muted">{agent.agency_name}</div><div className={overdue || dueToday ? "dangerText" : "muted"}>{daysLabel} · {agent.relationship_status || "No status"}</div></div><div className="rowActions"><button onClick={() => openCallChoice(agent)}>Call</button>{agent.agent_email && <a className="rowEmailButton" href={emailLink(agent)}>Email</a>}<button onClick={() => openAgent(agent)}>Open</button></div></div>;
}
...
function AgentProfile({ agent, engagements, close, openQuickAction, openCallChoice, updateStatus, archiveAgent, deleteAgent, emailLink }) {
  ...
  return <div className="modalOverlay"><div className="profileSheet"><div className="sheetHeader"><button onClick={close}>Close</button><strong>Agent Profile</strong><span /></div><div className="profileHero"><div className="avatar">{agent.name.slice(0, 1).toUpperCase()}</div><h2>{agent.name}</h2><p>{agent.agency_name}</p><div className="profileStats"><Metric label="Score" value={agent.score} /><Metric label="Referrals" value={agent.referral_count || 0} /><Metric label="Days" value={agent.daysRemaining === null ? "-" : agent.daysRemaining < 0 ? `${Math.abs(agent.daysRemaining)} late` : agent.daysRemaining} /></div></div><div className="quickGrid">{(agent.agent_phone || agent.main_phone) && <button onClick={() => openCallChoice(agent)}>Call</button>}{agent.agent_phone && <a href={`sms:${agent.agent_phone}`}>Text</a>}{agent.agent_email && <a href={emailLink(agent)}>Email</a>}{address && <a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(address)}`} target="_blank" rel="noreferrer">Map</a>}</div><div className="quickGrid"><button onClick={() => openQuickAction(agent, "Phone Call")}>Log Call</button><button onClick={() => openQuickAction(agent, "Office Stop In")}>Log Stop In</button><button onClick={() => openQuickAction(agent, "Referral Request")}>Referral Ask</button><button onClick={() => openQuickAction(agent, "Referral Received")}>Referral Won</button></div><div className="card"><label>Status</label><select value={agent.relationship_status || "New Prospect"} onChange={(e) => updateStatus(agent, e.target.value)}>{STATUS_OPTIONS.map((status) => <option key={status}>{status}</option>)}</select></div><div className="card"><h3>Memory</h3><p>{agent.notes || "No memory notes yet."}</p>{agent.favorite_food && <p><strong>Food/Drink:</strong> {agent.favorite_food}</p>}{agent.birthday && <p><strong>Birthday:</strong> {agent.birthday}</p>}{tags.length > 0 && <div className="tagWrap">{tags.map((tag) => <span key={tag}>{tag}</span>)}</div>}</div><div className="card"><h3>Engagement Timeline</h3>{engagements.length === 0 ? <p>No history yet.</p> : engagements.map((item) => <div className="timelineItem" key={item.id}><div className="timelineTop"><strong>{item.engagement_type}</strong><span>{item.engagement_date}</span></div>{item.action_taken && <p><strong>Action:</strong> {item.action_taken}</p>}{item.notes && <p>{item.notes}</p>}{item.outcome && <p><strong>Outcome:</strong> {item.outcome}</p>}{item.next_action && <p><strong>Next:</strong> {item.next_action}</p>}</div>)}</div><div className="dangerZone"><button onClick={() => archiveAgent(agent)}>Move to Do Not Pursue</button><button onClick={() => deleteAgent(agent)}>Delete Permanently</button></div></div></div>;
}

function CallChoiceModal({ agent, close, openQuickAction }) {
  return <div className="modalOverlay"><div className="actionSheet"><div className="sheetHeader"><button type="button" onClick={close}>Cancel</button><strong>Choose Number</strong><span /></div><p className="modalSub">{displayAgentName(agent)} · {agent.agency_name}</p><div className="callChoiceList">{agent.agent_phone ? <a className="callChoiceButton" href={`tel:${agent.agent_phone}`}>Call Mobile: {agent.agent_phone}</a> : <div className="disabledChoice">No mobile phone saved</div>}{agent.main_phone ? <a className="callChoiceButton" href={`tel:${agent.main_phone}`}>Call Office: {agent.main_phone}</a> : <div className="disabledChoice">No office phone saved</div>}<button className="secondaryFullButton" type="button" onClick={() => { close(); openQuickAction(agent, "Phone Call"); }}>Log Phone Call Notes</button></div></div></div>;
}

function EngagementModal({ agent, actionType, form, setForm, saving, saveEngagement, close }) {
  return <div className="modalOverlay"><form className="actionSheet" onSubmit={saveEngagement}><div className="sheetHeader"><button type="button" onClick={close}>Cancel</button><strong>{actionType}</strong><span /></div><p className="modalSub">{displayAgentName(agent)} · {agent.agency_name}</p>{actionType === "Office Stop In" && <Select label="Action Taken" value={form.action_taken} options={OFFICE_ACTIONS} onChange={(value) => setForm({ ...form, action_taken: value })} />}{actionType !== "Office Stop In" && actionType !== "Phone Call" && <Input label="Action Taken" value={form.action_taken} onChange={(value) => setForm({ ...form, action_taken: value })} placeholder="Short label" />}<Textarea label="Engagement Notes" value={form.notes} onChange={(value) => setForm({ ...form, notes: value })} placeholder="What happened?" required /><Textarea label="Outcome" value={form.outcome} onChange={(value) => setForm({ ...form, outcome: value })} placeholder="Warmer, neutral, needs another touch, etc." /><Input label="Next Action" value={form.next_action} onChange={(value) => setForm({ ...form, next_action: value })} placeholder="Stop by next month with coffee" /><button className="primaryButton" disabled={saving}>{saving ? "Saving..." : "Save Engagement"}</button></form></div>;
}

function displayAgentName(agent) { return [agent.agent_first_name, agent.agent_last_name].filter(Boolean).join(" ") || agent.agency_name || "Unnamed Agent"; }
function Section({ title, subtitle, children }) { return <section className="sectionCard"><div className="sectionHead"><h2>{title}</h2><p>{subtitle}</p></div><div className="sectionBody">{children}</div></section>; }
function Metric({ label, value, danger }) { return <div className={`metric ${danger ? "dangerMetric" : ""}`}><span>{label}</span><strong>{value}</strong></div>; }
function EmptyState({ text }) { return <div className="emptyState">{text}</div>; }
function Play({ title, text }) { return <div className="play"><h3>{title}</h3><p>{text}</p></div>; }
function Input({ label, value, onChange, required = false, type = "text", placeholder = "" }) { return <label className="field"><span>{label}{required ? " *" : ""}</span><input type={type} value={value || ""} onChange={(e) => onChange(e.target.value)} required={required} placeholder={placeholder} /></label>; }
function Textarea({ label, value, onChange, required = false, placeholder = "" }) { return <label className="field"><span>{label}{required ? " *" : ""}</span><textarea value={value || ""} onChange={(e) => onChange(e.target.value)} required={required} placeholder={placeholder} /></label>; }
function Select({ label, value, options, onChange }) { return <label className="field"><span>{label}</span><select value={value || ""} onChange={(e) => onChange(e.target.value)}>{options.map((option) => <option key={option}>{option}</option>)}</select></label>; }
function BottomNav({ activeTab, setActiveTab }) { const items = [["mission", "Mission"], ["agents", "Agents"], ["add", "Add"], ["partners", "Partners"], ["playbook", "Playbook"]]; return <nav className="bottomNav">{items.map(([key, label]) => <button key={key} onClick={() => setActiveTab(key)} className={activeTab === key ? "active" : ""}>{label}</button>)}</nav>; }
function parseTagsSafe(tags) { if (!tags) return []; return String(tags).split(",").map((tag) => tag.trim()).filter(Boolean); }
...
.smallPrimary { padding: 12px; font-size: 14px; }
...
@media (max-width: 640px) { .screen { padding: 10px 10px 98px; } .topHeader { border-radius: 0 0 22px 22px; padding: 15px; } h1 { font-size: 24px; } .metricGrid { grid-template-columns: repeat(2, 1fr); } .agentRow { align-items: stretch; flex-direction: column; } .rowActions { display: grid; grid-template-columns: 1fr 1fr 1fr; } .profileStats, .quickGrid { grid-template-columns: repeat(2, 1fr); } .filterRow, .twoCol { grid-template-columns: 1fr; } .bottomNav { bottom: 8px; } .bottomNav button { font-size: 11px; padding: 10px 2px; } }
`;
  const [callChoiceAgent, setCallChoiceAgent] = useState(null);
  const [quickActionType, setQuickActionType] = useState("");
  const [engagementForm, setEngagementForm] = useState(EMPTY_ENGAGEMENT);
...
function AgentsScreen({ title, agents, search, setSearch, statusFilter, setStatusFilter, sortBy, setSortBy, setDirectoryTitle, openAgent, openCallChoice, emailLink }) {
  function showAllAgents() {
    ...
    setStatusFilter("All");
    setSortBy("Name A-Z");
    setDirectoryTitle("All Saved Agents");
  }

  return (
    <div className="stack">
      <div className="filterCard">
        <button className="primaryButton smallPrimary" type="button" onClick={showAllAgents}>Show All Saved Agents</button>
      ...
      <Section title={`${title || "Agent Directory"} (${agents.length})`} subtitle="Tap any row to view the full profile and history.">
        {agents.length === 0 ? (
          <div className="emptyState">
            <p>No agents found for the current filter.</p>
            <button className="secondaryFullButton" type="button" onClick={showAllAgents}>Reset and Show All Agents</button>
          </div>
        ) : (
          agents.map((agent) => <AgentRow key={agent.id} agent={agent} openAgent={openAgent} openCallChoice={openCallChoice} emailLink={emailLink} />)
        )}
      </Section>
    </div>
  );
