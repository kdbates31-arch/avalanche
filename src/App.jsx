...
function getStatusBadgeStyle(status) {
  ...
  const styles = {
    "New Prospect": { background: "#075985", color: "#BAE6FD" },
    "Needs Follow Up": { background: "#7C2D12", color: "#FDBA74" },
    "Warm Relationship": { background: "#713F12", color: "#FDE68A" },
    "Maintenance Relationship": { background: "#1E3A8A", color: "#BFDBFE" },
    "Active Referral Partner": { background: "#14532D", color: "#BBF7D0" },
    "VIP Referral Partner": { background: "#78350F", color: "#FDE68A" },
    Cold: { background: "#374151", color: "#D1D5DB" },
    "Do Not Pursue": { background: "#111827", color: "#9CA3AF" }
  };

  return { ...base, ...(styles[status] || styles.Cold) };
}
        ...
        {!loading && tab === "contacts" && (
          <div style={styles.stack}>
            <div style={styles.card}>
              ...
              <select style={styles.input} value={strengthFilter} onChange={(e) => setStrengthFilter(e.target.value)}><option>All Trust Levels</option>{RELATIONSHIP_STRENGTH.map((x) => <option key={x}>{x}</option>)}</select>
            ...
            <Section title={`Contact Directory (${directory.length})`} subtitle="Search or filter by status, company type, referral source, role, or trust level.">
    ...
    <div style={styles.overlay}>
      <div style={styles.sheet}>
        ...
        <div style={styles.cardInner}>
          ...
          <p><b>Address:</b> {address || "Not saved"}</p>
        </div>
...
function ContactForm({ form, setForm, onSubmit, saving, nextFollowUp, today, buttonText }) {
  function set(key, value) { setForm({ ...form, [key]: value }); }
  return <form style={styles.form} onSubmit={onSubmit}><Select label="Relationship Status" value={form.relationship_status} options={STATUSES} onChange={(v) => setForm({ ...form, relationship_status: v, next_follow_up_date: nextFollowUp(v, form.last_contact_date || today()) || "" })} /><Input label="Company Name" value={form.agency_name} onChange={(v) => set("agency_name", v)} required /><Input label="First Name" value={form.agent_first_name} onChange={(v) => set("agent_first_name", v)} /><Input label="Last Name" value={form.agent_last_name} onChange={(v) => set("agent_last_name", v)} /><Input label="Mobile Phone" value={form.agent_phone} onChange={(v) => set("agent_phone", v)} /><Input label="Office Phone" value={form.main_phone} onChange={(v) => set("main_phone", v)} /><Input label="Email" value={form.agent_email} onChange={(v) => set("agent_email", v)} /><Input label="Address" value={form.address} onChange={(v) => set("address", v)} /><Input label="City" value={form.city} onChange={(v) => set("city", v)} /><Input label="State" value={form.state} onChange={(v) => set("state", v)} /><Select label="Company Type" value={form.company_type} options={COMPANY_TYPES} onChange={(v) => set("company_type", v)} /><DatalistInput label="Contact Role" value={form.contact_role} onChange={(v) => set("contact_role", v)} suggestions={CONTACT_ROLE_SUGGESTIONS} /><Select label="Referral Source" value={form.referral_source} options={REFERRAL_SOURCES} onChange={(v) => set("referral_source", v)} /><Select label="Trust Level" value={form.relationship_strength} options={RELATIONSHIP_STRENGTH} onChange={(v) => set("relationship_strength", v)} /><Input label="Introduced By" value={form.introduced_by} onChange={(v) => set("introduced_by", v)} /><Input type="date" label="Last Contact" value={form.last_contact_date} onChange={(v) => setForm({ ...form, last_contact_date: v, next_follow_up_date: nextFollowUp(form.relationship_status, v) || "" })} /><Input type="date" label="Next Follow Up" value={form.next_follow_up_date} onChange={(v) => set("next_follow_up_date", v)} /><Input label="Favorite Food / Drink" value={form.favorite_food} onChange={(v) => set("favorite_food", v)} /><Input label="Birthday" value={form.birthday} onChange={(v) => set("birthday", v)} /><Input label="Tags" value={form.tags} onChange={(v) => set("tags", v)} /><Textarea label="Memory Notes" value={form.notes} onChange={(v) => set("notes", v)} /><button style={styles.primary} disabled={saving}>{saving ? "Saving..." : buttonText}</button></form>;
}
...
function DatalistInput({ label, value, onChange, suggestions }) {
  ...
  return (
    <label style={styles.label}>{label}
      <input style={styles.input} list={listId} value={value || ""} onChange={(e) => onChange(e.target.value)} />
      <datalist id={listId}>
