...
export default function App() {
  ...
  async function updateReferralLead(e) {
    ...
    if (payload.referral_status === "Lost") {
      ...
      if (lostReasonHasDrip(payload.lost_reason)) {
    ...
    <div style={{ ...styles.row, borderLeft: `6px solid ${TILE_ACCENTS.sync}` }}>
      ...
      {updateReferralStatus && (
        <div style={styles.actionRowSmall}>
          ...
          {status === "Lost" && lostReasonHasDrip(referral.lost_reason || "Insurance Denied") && sendLostDripEmail && referral.customer_email && <button style={actionStyle("email")} onClick={(e) => { e.stopPropagation(); sendLostDripEmail(referral); }}>{lostDripLabel(referral)}</button>}
        </div>
      )}
    </div>
  );
