        ...
        {!loading && tab === "contacts" && (
          <div style={styles.stack}>
            ...
            <div style={styles.card}>
              ...
              <select style={styles.input} value={originFilter} onChange={(e) => setOriginFilter(e.target.value)}><option>All Contact Origins</option>{CONTACT_ORIGINS.map((x) => <option key={x}>{x}</option>)}</select>
              ...
              <select style={styles.input} value={roleFilter} onChange={(e) => setRoleFilter(e.target.value)}><option>All Roles</option>{roleOptions.map((x) => <option key={x}>{x}</option>)}</select>
            </div>
        ...
        {!loading && tab === "add" && (
          <div style={styles.stack}>
            ...
            {addMode === "relationship" ? (
              <ContactForm form={agentForm} setForm={setAgentForm} onSubmit={saveAgent} saving={saving} nextFollowUp={nextFollowUp} today={today} buttonText="Save Relationship Contact" />
            ) : (
...
function EditModal({ form, setForm, updateAgent, saving, close, nextFollowUp, today }) {
  return <div style={styles.overlay}><div style={styles.sheet}><button style={actionStyle("neutral")} onClick={close}>Cancel</button><h2>Edit Relationship Contact</h2><ContactForm form={form} setForm={setForm} onSubmit={updateAgent} saving={saving} nextFollowUp={nextFollowUp} today={today} buttonText="Update Relationship Contact" /></div></div>;
}
