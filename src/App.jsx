...
export default function App() {
  ...
  async function updateReferralStatus(referral, nextStatus) {
    ...
    const { error } = await supabase
      .from("referral_leads")
      .update({ referral_status: nextStatus, updated_at: today() })
      .eq("id", referral.id);
  ...
  const activeReferralLeads = referrals.filter((item) => (item.referral_status || "New Referral") !== "Archived");
  ...
  const referralBuckets = useMemo(() => {
    ...
    activeReferralLeads.forEach((ref) => {
      ...
      if (ref.referral_status === "Contract Signed") bucket.signed += 1;
      if (["New Referral", "Working", "Estimate Scheduled"].includes(ref.referral_status || "New Referral")) bucket.working += 1;
      if (ref.referral_status === "Lost") bucket.lost += 1;
    });
  ...
  return (
    <div style={styles.app}>
      ...
      {selectedReferral && <ReferralProfile referral={selectedReferral} close={() => setSelectedReferral(null)} edit={() => { setEditingReferral(selectedReferral); setSelectedReferral(null); }} updateReferralStatus={updateReferralStatus} archiveReferral={archiveReferral} />}
...
function ReferralPipeline({ buckets, selectedBucket, selectedReferralSource, setSelectedReferralSource, openReferral, updateReferralStatus }) {
  const visibleBuckets = buckets || [];
  return (
    <div style={styles.stack}>
      <Section title="Referral Pipeline" subtitle="Referral leads grouped by agency and the person who sent them.">
        {visibleBuckets.length === 0 ? <p>No active referral leads yet.</p> : visibleBuckets.map((bucket) => (
          <button key={bucket.key} style={selectedReferralSource === bucket.key ? styles.pipelineBucketOn : styles.pipelineBucket} onClick={() => setSelectedReferralSource(selectedReferralSource === bucket.key ? null : bucket.key)}>
            ...
            <div style={styles.pipelineStats}>
              <span>{bucket.referrals.length} total</span>
              <span>{bucket.working} active</span>
              <span>{bucket.signed} signed</span>
            </div>
          </button>
        ))}
      </Section>

      <Section title={selectedBucket ? `${selectedBucket.sent_by_name} Referrals` : "All Active Referrals"} subtitle="Open a referral or update the outcome directly from the pipeline.">
        {(selectedBucket ? selectedBucket.referrals : visibleBuckets.flatMap((bucket) => bucket.referrals)).length === 0 ? <p>No referrals in this bucket.</p> : (selectedBucket ? selectedBucket.referrals : visibleBuckets.flatMap((bucket) => bucket.referrals)).map((ref) => (
...
function ReferralRow({ referral, open, updateReferralStatus }) {
...
function ReferralProfile({ referral, close, edit, updateReferralStatus, archiveReferral }) {
  const status = referral.referral_status || "New Referral";
  return (
    <div style={styles.overlay}>
      <div style={styles.sheet}>
