...
function lostDripStep(referral) {
  ...
  return ["30", "60", "90", "Quarterly"].includes(step) ? step : "30";
}

function lostDripLabel(referral) {
  const step = lostDripStep(referral);
  return step === "Quarterly" ? "Quarterly Email" : `${step}-Day Email`;
}
...
function nextLostDripDateAfterSend(currentStep, baseDate) {
  ...
  return addDaysStatic(baseDate, 90);
}

function lostDripEmailSubject(referral) {
  const step = lostDripStep(referral);
  if (step === "30") return "Just checking back in from Cool Roofs";
  if (step === "60") return "Staying in touch from Cool Roofs";
  if (step === "90") return "Quick follow-up from Cool Roofs";
  return "Checking in from Cool Roofs";
}
...
I wanted to check back in now that a little time has passed. I know roofing decisions can get paused for all kinds of reasons: timing, insurance questions, budget, schedules, or just needing more time to think it through.

No pressure at all. I just wanted to stay on your radar in case anything has changed or if you still have questions I can help with.
...
I wanted to touch base again since it has been a couple of months. Sometimes roofing conversations get put on the back burner, and I did not want to assume the need had completely gone away.

If everything has already been handled, no problem at all. If you are still weighing options or just need a quick second look, I would be glad to help.
...
I wanted to circle back one more time now that some real time has passed. I am not trying to push anything. I just like to stay connected in case timing, insurance, or priorities have changed.

If the roof has already been taken care of, I hope everything went smoothly. If not, or if you ever need another set of eyes on it, I am happy to be a resource.
...
I wanted to stay in touch and keep myself available as a resource. No pressure at all. If anything ever changes with the roof, insurance, or property plans, I would be glad to help.
...
export default function App() {
  ...
  async function saveReferralLead(e) {
    ...
    const { error } = await supabase.from("referral_leads").insert([lead]);
  ...
  async function updateReferralLead(e) {
    ...
    setSaving(true);
    const { id, ...payload } = { ...editingReferral, updated_at: today() };
    const { error } = await supabase.from("referral_leads").update(payload).eq("id", editingReferral.id);
  ...
  async function updateReferralStatus(referral, nextStatus) {
    if (!referral) return;
    const updates = { referral_status: nextStatus, updated_at: today() };

    if (nextStatus === "Lost") {
      updates.lost_drip_next_date = addDays(today(), 30);
      updates.lost_drip_last_sent_date = referral.lost_drip_last_sent_date || null;
      updates.lost_drip_step = referral.lost_drip_step || "30";
    }

    if (nextStatus === "Contract Signed") {
      updates.lost_drip_next_date = null;
      updates.lost_drip_last_sent_date = null;
    ...
    await loadReferrals();
    goToTab("referrals");
    setMessage(nextStatus === "Lost" ? "Referral moved to Lost. 30-day follow-up date created." : `Referral marked ${nextStatus}.`);
  }
  ...
  async function sendLostDripEmail(referral) {
    ...
    if (error) {
      ...
      const nextLabel = nextStep === "Quarterly" ? "quarterly" : `${nextStep}-day`;
      setMessage(`${currentStep}-day lost follow-up email opened. Next ${nextLabel} follow-up date updated.`);
      await loadReferrals();
    }
...
function ReferralPipeline({ buckets, selectedBucket, selectedReferralSource, setSelectedReferralSource, openReferral, updateReferralStatus, signedReferrals = [], lostReferrals = [], archivedReferrals = [], archiveSearch, setArchiveSearch, referralMatchesSearch, sendLostDripEmail }) {
  ...
  const activeList = selectedBucket ? selectedBucket.referrals : visibleBuckets.flatMap((bucket) => bucket.referrals);
    ...
    <div style={styles.stack}>
      <Section title="Referral Pipeline" subtitle="Active referral leads grouped by agency and the person who sent them.">
        {visibleBuckets.length === 0 ? <p>No active referral leads yet.</p> : visibleBuckets.map((bucket) => (
          <button key={bucket.key} style={selectedReferralSource === bucket.key ? styles.pipelineBucketOn : styles.pipelineBucket} onClick={() => setSelectedReferralSource(selectedReferralSource === bucket.key ? null : bucket.key)}>
            <div>
            ...
            <div style={styles.pipelineStats}>
              ...
              <span>{bucket.working} working</span>
            </div>
          </button>
        ))}
      </Section>

      <Section title={selectedBucket ? `${selectedBucket.sent_by_name} Active Referrals` : "All Active Referrals"} subtitle="Open a referral or update the current outcome directly from the pipeline.">
        {activeList.length === 0 ? <p>No active referrals in this bucket.</p> : activeList.map((ref) => (
...
function ReferralRow({ referral, open, updateReferralStatus, sendLostDripEmail }) {
  ...
  return (
    <div style={{ ...styles.row, borderLeft: `6px solid ${TILE_ACCENTS.sync}` }}>
      <div onClick={open} style={{ cursor: "pointer" }}>
        ...
        <div style={styles.actionRowSmall}><ReferralStatusBadge status={status} /></div>
      </div>
      {updateReferralStatus && (
        <div style={styles.actionRowSmall}>
          <button style={actionStyle("neutral")} onClick={(e) => { e.stopPropagation(); updateReferralStatus(referral, "Working"); }}>Working</button>
          <button style={actionStyle("ask")} onClick={(e) => { e.stopPropagation(); updateReferralStatus(referral, "Estimate Scheduled"); }}>Estimate</button>
          <button style={actionStyle("won")} onClick={(e) => { e.stopPropagation(); updateReferralStatus(referral, "Contract Signed"); }}>Signed</button>
          <button style={actionStyle("caution")} onClick={(e) => { e.stopPropagation(); updateReferralStatus(referral, "Lost"); }}>Lost</button>
          {status === "Lost" && sendLostDripEmail && referral.customer_email && <button style={actionStyle("email")} onClick={(e) => { e.stopPropagation(); sendLostDripEmail(referral); }}>{lostDripLabel(referral)}</button>}
        </div>
      )}
    </div>
  );
    ...
    <div style={styles.overlay}>
      <div style={styles.sheet}>
        ...
        <div style={styles.cardInner}>
          ...
          {status === "Lost" && <p><b>Next Lost Follow-Up:</b> {referral.lost_drip_next_date || "Not scheduled"} · {lostDripLabel(referral)}</p>}
        </div>
        <div style={styles.cardInner}>
        ...
        <div style={styles.actionRow}>
          {status === "Lost" && referral.customer_email && <button style={actionStyle("email")} onClick={() => sendLostDripEmail(referral)}>{lostDripLabel(referral)}</button>}
  ...
  return (
    <form style={styles.form} onSubmit={onSubmit}>
      ...
      <Section title="Referral Status" subtitle="Track what happened to this referral lead.">
        <Select label="Referral Status" value={form.referral_status || "New Referral"} options={REFERRAL_STATUSES} onChange={(v) => set("referral_status", v)} />
      </Section>
...
const styles = {
  ...
  pipelineBucket: { background: COLORS.innerBg, color: COLORS.text, border: `1px solid ${COLORS.border}`, borderLeft: `5px solid ${TILE_ACCENTS.sync}`, borderRadius: 14, padding: 12, textAlign: "left", display: "flex", justifyContent: "space-between", gap: 10, alignItems: "center" },
  pipelineBucketOn: { background: "#24145f", color: COLORS.text, border: `1px solid ${TILE_ACCENTS.sync}`, borderLeft: `5px solid ${TILE_ACCENTS.sync}`, borderRadius: 14, padding: 12, textAlign: "left", display: "flex", justifyContent: "space-between", gap: 10, alignItems: "center" },
