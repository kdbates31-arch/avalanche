...
export default function App() {
  ...
  const dueNow = sortOldestTouchFirst(activeAgents.filter((a) => a.relationship_status !== "VIP Referral Partner" && isContactDueByStatus(a)));
      ...
      <main style={styles.screen}>
        ...
        {!loading && tab === "mission" && (
          <div style={styles.stack}>
            ...
            <BucketSection
              title="15+ Day Touches"
              subtitle="New prospects and needs-follow-up contacts ready for another touch. Oldest touch is listed first."
              contacts={follow15}
              emptyText="No 15+ day touches right now."
              setSelectedAgent={setSelectedAgent}
              setCallAgent={setCallAgent}
              emailAndLog={emailAndLog}
            />

            <BucketSection
              title="30+ Day Touches"
              subtitle="Warm relationships ready for another touch. Oldest touch is listed first."
              contacts={warm30}
              emptyText="No 30+ day touches right now."
              setSelectedAgent={setSelectedAgent}
              setCallAgent={setCallAgent}
              emailAndLog={emailAndLog}
            />
            ...
            <BucketSection
              title="60+ Day Touches"
              subtitle="Maintenance relationships ready for a light check-in. Oldest touch is listed first."
              contacts={maintenance60}
              emptyText="No 60+ day touches right now."
              setSelectedAgent={setSelectedAgent}
              setCallAgent={setCallAgent}
              emailAndLog={emailAndLog}
            />
            ...
            <BucketSection
              title="VIP Relationship Touches"
              subtitle="Soft reminders for strategic relationships, not sales pressure. Oldest touch is listed first."
              contacts={vipSuggestions}
              emptyText="No VIP relationship touches suggested right now."
              setSelectedAgent={setSelectedAgent}
              setCallAgent={setCallAgent}
              emailAndLog={emailAndLog}
              soft
...
function BucketSection({ title, subtitle, contacts, emptyText, setSelectedAgent, setCallAgent, emailAndLog, soft }) {
  return (
      ...
      {contacts.length === 0 ? (
        ...
        contacts.map((agent) => (
          <ContactRow
            key={agent.id}
            agent={agent}
            open={() => setSelectedAgent(agent)}
            openCall={() => setCallAgent(agent)}
            emailAndLog={() => emailAndLog(agent)}
            soft={soft}
          />
        ))
      )}
      ...
      <div style={styles.summaryGrid}>
        {items.map((item) => (
          <button key={item[labelKey]} style={styles.summaryCard} onClick={() => onClick(item[labelKey])}>
            <div style={{ marginBottom: 10 }}>{item[labelKey]}</div>
            <b style={{ fontSize: 28 }}>{item.count}</b>
          </button>
        ))}
      </div>
    </Section>
