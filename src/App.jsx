...
export default function App() {
  ...
  async function updateAgent(e) {
    ...
    const contactId = editingAgent.id;
    if (!contactId) {
      setMessage("Update failed: missing contact ID. Close and reopen this contact before editing.");
      return;
    }
    ...
    try {
      ...
      let finalFront = editingAgent.business_card_front || "";
      let finalBack = editingAgent.business_card_back || "";
      let photoMessage = "";

      try {
        const selectedFront = editingCardPhotos?.front || "";
        const selectedBack = editingCardPhotos?.back || "";
        const frontChanged = selectedFront !== finalFront;
        const backChanged = selectedBack !== finalBack;

        if (frontChanged && selectedFront) {
          const uploadedFront = await uploadBusinessCardImage(selectedFront, String(contactId), "front");
          finalFront = uploadedFront.url || "";
        } else if (frontChanged && !selectedFront) {
          finalFront = "";
        }

        if (backChanged && selectedBack) {
          const uploadedBack = await uploadBusinessCardImage(selectedBack, String(contactId), "back");
          finalBack = uploadedBack.url || "";
        } else if (backChanged && !selectedBack) {
          finalBack = "";
        }
      } catch (photoError) {
        photoMessage = " Business card photos did not save: " + (photoError?.message || String(photoError));
      }

      const updatePayload = {
        ...payload,
        business_card_front: finalFront,
        business_card_back: finalBack
      };
      ...
      const { data: updatedRows, error } = await supabase
        ...
        .eq("id", contactId)
        .select("id, business_card_front, business_card_back");
      ...
      if (!updatedRows || updatedRows.length === 0) {
        setMessage("Update failed: no matching contact was found. Close and reopen this contact before editing.");
        return;
      }

      const finalUpdateMessage = photoMessage
        ? "Relationship Contact updated." + photoMessage
        : "Relationship Contact updated. Business card photos saved.";
