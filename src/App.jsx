...
export default function App() {
  ...
  const directory = useMemo(() => {
    ...
    if (statusFilter !== "All") list = list.filter((a) => a.relationship_status === statusFilter);
        ...
        {!loading && tab === "mission" && (
          <div style={styles.stack}>
            ...
            <Section title="VIP Relationship Suggestions" subtitle="Soft reminders, not sales pressure.">
              {vipSuggestions.length === 0 ? <p>No VIP suggestions right now.</p> : vipSuggestions.slice(0, 10).map((agent) => <ContactRow key={agent.id} agent={agent} open={() => setSelectedAgent(agent)} openCall={() => setCallAgent(agent)} emailAndLog={() => emailAndLog(agent)} soft />)}
            </Section>
        ...
        {!loading && tab === "contacts" && (
          <div style={styles.stack}>
            <div style={styles.card}>
              ...
              <select style={styles.input} value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}><option>All</option>{STATUSES.map((x) => <option key={x}>{x}</option>)}</select>
...
function Tile({ label, value, onClick, danger }) {
  return <button style={danger ? styles.tileDanger : styles.tile} onClick={onClick}><span>{label}</span><b>{value}</b></button>;
}
...
function Playbook() {
  return <Section title="Playbook" subtitle="Current cadence logic"><p><b>New Prospect:</b> 15 day hard follow-up.</p><p><b>Needs Follow Up:</b> 15 day hard follow-up.</p><p><b>Warm Relationship:</b> 30 day hard follow-up.</p><p><b>Maintenance Relationship:</b> 60 day hard follow-up.</p><p><b>Active Referral Partner:</b> no automated sales trigger.</p><p><b>VIP Referral Partner:</b> 45 day soft relationship suggestion.</p></Section>;
}
