...
const styles = {
  ...
  async function changeStatus(agent, newStatus) {
    ...
    const { error } = await supabase
      .from("agencies")
      .update({ relationship_status: newStatus, next_follow_up_date: next })
      .eq("id", agent.id);
  ...
  const directory = useMemo(() => {
    ...
    if (q) {
      list = list.filter((a) => [a.name, a.agency_name, a.agent_phone, a.main_phone, a.agent_email, a.city, a.tags, a.notes, a.favorite_food]
        .filter(Boolean)
        .join(" ")
        .toLowerCase()
        .includes(q));
  ...
  return (
    <div className="app">
      ...
      <main className="screen">
        ...
        {!loading && tab === "mission" && (
          <div className="stack">
            ...
            <Section title="Priority Calls" subtitle="People who need contact now.">
              {due.length === 0 ? <Empty text="No overdue follow-ups." /> : due.slice(0, 12).map((a) => <AgentRow key={a.id} agent={a} openAgent={setSelectedAgent} openCall={setCallAgent} emailAndLog={emailAndLog} />)}
            </Section>
          </div>
        )}
        ...
        {!loading && tab === "agents" && (
          <div className="stack">
            ...
            <Section title={`Agent Directory (${directory.length})`} subtitle="Tap Open to see the profile and history.">
              {directory.length === 0 ? <Empty text="No agents match this filter." /> : directory.map((a) => <AgentRow key={a.id} agent={a} openAgent={setSelectedAgent} openCall={setCallAgent} emailAndLog={emailAndLog} />)}
            </Section>
          </div>
        )}
        ...
        {!loading && tab === "partners" && (
          <Section title="Referral Partners" subtitle="Protect these relationships.">
            {partners.length === 0 ? <Empty text="No referral partners yet." /> : partners.map((a) => <AgentRow key={a.id} agent={a} openAgent={setSelectedAgent} openCall={setCallAgent} emailAndLog={emailAndLog} />)}
          </Section>
        )}
      ...
      <nav className="nav">
        {[["mission", "Mission"], ["agents", "Agents"], ["add", "Add"], ["partners", "Partners"], ["playbook", "Playbook"]].map(([key, label]) => (
          <button key={key} className={tab === key ? "on" : ""} onClick={() => setTab(key)}>{label}</button>
        ))}
      </nav>

      {selectedAgent && <Profile agent={selectedAgent} logs={logs[selectedAgent.id] || []} close={() => setSelectedAgent(null)} openCall={setCallAgent} startLog={startLog} changeStatus={changeStatus} archiveAgent={archiveAgent} deleteAgent={deleteAgent} emailAndLog={emailAndLog} />}
...
function Profile({ agent, logs, close, openCall, startLog, changeStatus, archiveAgent, deleteAgent, emailAndLog }) {
