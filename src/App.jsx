...
async function uploadBusinessCardImage(value, folder, side) {
  ...
  if (error) throw new Error(`Business card ${side} upload failed: ${error.message}. Check Supabase Storage bucket permissions for business-cards.`);

  const { data } = supabase.storage.from(BUSINESS_CARD_BUCKET).getPublicUrl(path);
  return { url: data?.publicUrl || "", path };
}

async function uploadBusinessCardPhotos(cardPhotos, folder, existing = {}) {
  const currentFront = cardPhotos?.front || "";
  const currentBack = cardPhotos?.back || "";

  const front = currentFront
    ? isDataUrl(currentFront)
      ? await uploadBusinessCardImage(currentFront, folder, "front")
      : { url: currentFront, path: existing.frontPath || "" }
    : { url: "", path: "" };

  const back = currentBack
    ? isDataUrl(currentBack)
      ? await uploadBusinessCardImage(currentBack, folder, "back")
      : { url: currentBack, path: existing.backPath || "" }
    : { url: "", path: "" };
...
export default function App() {
  ...
  async function saveAgent(e) {
    ...
    try {
      ...
      const photoFolder = `new_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
      const uploadedPhotos = await uploadBusinessCardPhotos(agentCardPhotos, photoFolder);
      const payload = {
        ...agentForm,
        ...
        business_card_front: "",
        business_card_back: "",
        business_card_front_url: uploadedPhotos.front.url,
        business_card_back_url: uploadedPhotos.back.url,
        business_card_front_path: uploadedPhotos.front.path,
        business_card_back_path: uploadedPhotos.back.path
      };
  ...
  function startEdit(agent) {
    setEditingCardPhotos({
      front: agent.business_card_front_url || agent.business_card_front || getCardPhotos(agent.id).front || "",
      back: agent.business_card_back_url || agent.business_card_back || getCardPhotos(agent.id).back || ""
    });
    setEditingAgent({
      ...EMPTY_AGENT,
      ...agent,
  ...
  async function updateAgent(e) {
    ...
    try {
      ...
      const uploadedPhotos = await uploadBusinessCardPhotos(editingCardPhotos, String(editingAgent.id || "contact"), {
        frontPath: editingAgent.business_card_front_path || "",
        backPath: editingAgent.business_card_back_path || ""
      });

      payload.business_card_front = "";
      payload.business_card_back = "";
      payload.business_card_front_url = uploadedPhotos.front.url;
      payload.business_card_back_url = uploadedPhotos.back.url;
      payload.business_card_front_path = uploadedPhotos.front.path;
      payload.business_card_back_path = uploadedPhotos.back.path;
      ...
      setMessage("Relationship Contact updated. Business card photos saved.");
  ...
  return (
    <div style={styles.app}>
        ...
        {!loading && tab === "mission" && (
          <div style={styles.stack}>
            <div style={styles.tiles}>
              ...
              <Tile label="Pending JobNimbus Sync" value={pendingSync.length} onClick={() => goToTab("contacts")} accent={TILE_ACCENTS.sync} />
            </div>
            ...
            {referralAttributionSummary.length > 0 && <SummarySection title="Referral Attribution Snapshot" subtitle="Who is sending roofing opportunities into Avalanche." items={referralAttributionSummary} labelKey="source" onClick={() => goToTab("contacts")} />}
      ...
      <nav style={styles.nav}>
        ...
        <button style={tab === "contacts" ? styles.navOn : styles.navButton} onClick={() => goToTab("contacts")}>Contacts</button>
      ...
      {selectedAgent && <ContactProfile agent={selectedAgent} cardPhotos={{ front: selectedAgent.business_card_front_url || selectedAgent.business_card_front || getCardPhotos(selectedAgent.id).front || "", back: selectedAgent.business_card_back_url || selectedAgent.business_card_back || getCardPhotos(selectedAgent.id).back || "" }} logs={logs[selectedAgent.id] || []} close={() => setSelectedAgent(null)} startEdit={startEdit} deleteAgent={deleteAgent} moveToDoNotPursue={moveToDoNotPursue} openCall={() => setCallAgent(selectedAgent)} emailAndLog={() => emailAndLog(selectedAgent)} startLog={startLog} />}
...
function BusinessCardUpload({ cardPhotos, setCardPhotos }) {
  ...
  return (
    <Section title="Business Card Photos" subtitle="Upload or take a front and back photo. Photos save to Supabase Storage to avoid quota errors.">
      <div style={styles.cardPhotoGrid}>
