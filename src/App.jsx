...
export default function App() {
  ...
  async function saveAgent(e) {
    ...
    const { data: inserted, error } = await supabase.from("agencies").insert([payload]).select("id").single();

    if (error) {
      setMessage("Save failed: " + error.message);
    } else {
      if (inserted?.id) saveCardPhotos(inserted.id, agentCardPhotos);
  ...
  function startEdit(agent) {
    setEditingCardPhotos(getCardPhotos(agent.id));
    setEditingAgent({
      ...EMPTY_AGENT,
      ...agent,
...
function BusinessCardUpload({ cardPhotos, setCardPhotos }) {
  function handleFile(side, file) {
    ...
    reader.onload = () => {
      setCardPhotos({ ...(cardPhotos || EMPTY_CARD_PHOTOS), [side]: reader.result });
    };
  ...
  return (
    <Section title="Business Card Photos" subtitle="Upload or take a front and back photo. Auto-crop and OCR can be added later.">
      <div style={styles.cardPhotoGrid}>
